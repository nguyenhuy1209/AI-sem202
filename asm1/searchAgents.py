...
class CornersProblem(search.SearchProblem):
    ...
    def getNextState(self, state, action):
        ...
        nextx, nexty = int(x + dx), int(y + dy)
        "*** YOUR CODE HERE ***"
        next_corner_state = state[1].copy()
        for i in range(len(state[1])):
            if (nextx,nexty) == state[1][i]:
                del next_corner_state[i]
                break
        return ((nextx, nexty), next_corner_state)
...
def minimumManhattanDistance(currentPosition, corners, walls):
    distance = [util.manhattanDistance(currentPosition, corner)  for corner in corners]
...
def cornersHeuristic(state, problem):
    ...
    corners = state[1]
    while corners != []:
        min_distance, indx_min_distance = minimumManhattanDistance(currentPosition, corners, walls)
        heuristic += min_distance
        currentPosition = corners[indx_min_distance]
        corners = corners[:indx_min_distance] + corners[indx_min_distance+1:]
    return heuristic
